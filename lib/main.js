...
Listener.prototype = {
    onChange: function (accessPoints) {
        ...
        if (sp.prefs.enableStumbling) {
            var geolocProvider = prefs.getCharPref('wifi.uri');
            var geolocProviderURI = Cc["@mozilla.org/network/io-service;1"].getService(Ci.nsIIOService).newURI(geolocProvider, null, null);
            var geolocProviderDomain = Cc["@mozilla.org/network/effective-tld-service;1"].getService(Ci.nsIEffectiveTLDService).getBaseDomain(geolocProviderURI);
